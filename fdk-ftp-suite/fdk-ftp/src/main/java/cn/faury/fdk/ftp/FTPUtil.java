...
public class FTPUtil {
     ...
     */
    public static void init(InputStream configFile) {
        logger.error("{}", "=====初始化FTP缓冲池组件 Start=====");
        if (pool == null) {
            if (configFile == null) {
                logger.error("{}", "》配置文件不可以为空=====");
                return;
            }
            synchronized (FTPUtil.class) {
                if (pool == null) {
                    logger.error("》开始读取配置文件[{}]=====", configFile);
                    // 初始化配置文件
                    ...
                    if (StringUtil.isEmpty(cfg.getProperty("ftp.hostname"), cfg.getProperty("ftp.port"),
                        ...
                        logger.error("Properties file [{}] error", configFile);
                    ...
                    logger.error("{}", "》FTP缓冲池组件初始化 Success=====");
                } else {
                    logger.error("{}", "》FTP缓冲池组件已经存在无需重复初始化=====");
                }
            }
        } else {
            logger.error("{}", "》FTP缓冲池组件已经存在无需重复初始化=====");
        }
        logger.error("{}", "=====初始化FTP缓冲池组件 Finish=====");
    }
     ...
     * @param ftpPool 缓冲池
     */
    public static void init(FTPPool ftpPool) {
        logger.error("{}", "=====初始化FTP缓冲池组件 Start=====");
        if (pool == null) {
            if (ftpPool == null) {
                logger.error("{}", "》配置缓冲池不可以为空=====");
                return;
            }
            synchronized (FTPUtil.class) {
                if (pool == null) {
                    logger.error("》开始FTP缓冲池组件初始化=====");
                    pool = ftpPool;
                    logger.error("》初始化账户参数：{}",
                            String.format("{hostname=%s,port=%s,username=%s,password=***,passiveModeConf=%s}",
                                    pool.getHostname(), pool.getPort(), pool.getUsername(), pool.isPassiveModeConf()));
                    logger.error("》初始化池参数：{}",
                            String.format(
                                    "{maxActive=%s,whenExhaustedAction=%s,maxWait=%s,testOnBorrow=%s,testOnReturn=%s}",
                                    pool.getPoolConfig().getMaxTotal(), pool.getPoolConfig().getBlockWhenExhausted()
                                    , pool.getPoolConfig().getMaxWaitMillis(), pool.getPoolConfig().getTestOnBorrow(),
                                    pool.getPoolConfig().getTestOnReturn()));
                    logger.error("{}", "》FTP缓冲池组件初始化 Success=====");
                } else {
                    logger.error("{}", "》FTP缓冲池组件已经存在无需重复初始化=====");
                }
    ...
    public static void destory() {
        logger.error("{}", "=====销毁FTP缓冲池组件 Start=====");
        if (pool != null) {
            pool.destory();
        }
        logger.error("{}", "=====销毁FTP缓冲池组件 End=====");
    }
    ...
    protected static void checkInit() {
        if (pool == null) {
            logger.debug("{}", "checkInit error[FTPPool not init]!");
    ...
    public static int getRemoteFileSize(String remote) {
        ...
        try {
            ...
            if (files.length < 1) {
                logger.error("FTP获取文件大小失败->找不到文件：{},{}", remote, ftpClient.getReplyString());
                return 0;
            }
            Long size = files[0].getSize();
            return size.intValue();
        } catch (IOException ex) {
            logger.error("FTP获取文件大小失败->获取文件出错：{},{}", remote, ftpClient.getReplyString(), ex);
            return 0;
        } finally {
    ...
    public static Date getRemoteFileDate(String remote) {
        ...
        try {
            ...
            FTPFile[] files = ftpClient.listFiles(remote);
            if (files.length < 1) {
                logger.error("FTP获取文件时间失败->找不到文件：{},{}", remote, ftpClient.getReplyString());
                return null;
            }
            Calendar date = files[0].getTimestamp();
            return date.getTime();
        } catch (IOException ex) {
            logger.error("FTP获取文件时间失败->获取文件出错：{},{}", remote, ftpClient.getReplyString(), ex);
            return null;
        } finally {
            returnInstance(ftpClient);
        }
    ...
    public static UploadStatus upload(HttpServletRequest request, String remote, boolean override) {
        ...
        if (request != null) {
            try {
                ...
                logger.error("{}", "FTP上传文件失败：读取文件流失败", e);
                result = UploadStatus.Upload_New_File_Failed;
            }
        } else {
            logger.error("{}", "FTP上传文件失败：输入参数request为空");
        }
        return result;
    }
    ...
    public static UploadStatus upload(InputStream stream, String remote, int offset, boolean override) {
        ...
        try {
            ...
            // 检查远程是否存在文件
            FTPFile[] files = ftpClient.listFiles(remote);
            logger.debug("检测目标文件是否存在：{}", files.length);
            if (files.length == 1) {
                logger.debug("{}", "文件已存在，进行断点续传......");
                result = _breakPointUpload(ftpClient, stream, remote, offset);
            } else {
                logger.debug("{}", "上传新文件......");
                if (ftpClient.storeFile(remote, stream)) {
                    logger.debug("{}", "上传新文件成功");
                    result = UploadStatus.Upload_New_File_Success;
                } else {
                    logger.debug("{}", "上传新文件失败");
                    result = UploadStatus.Upload_New_File_Failed;
                }
            }
            return result;
        } catch (RuntimeException | IOException ex) {
            logger.error("FTP上传文件失败：{},{}", remote, ftpClient.getReplyString(), ex);
            return UploadStatus.Upload_New_File_Failed;
        } finally {
            logger.debug("结束上传文件到：{}", remote);
            try {
                stream.close();
    ...
    public static byte[] download(String remote) {
        ...
        try {
            ...
            logger.error("FTP下载文件失败：{},{}", remote, ftpClient.getReplyString(), ex);
        } finally {
            returnInstance(ftpClient);
        }
        return null;
    ...
    public static boolean deleteFile(String filePath) {
        ...
        try {
            ...
            logger.error("FTP删除文件失败：{},{}", filePath, ftpClient.getReplyString(), ex);
        } finally {
            returnInstance(ftpClient);
        }
        return false;
    ...
    public static boolean moveFile(String fromPath, String toPath, boolean override) {
        ...
        try {
            ...
            logger.error("FTP移动文件失败：{} -> {},{}", fromPath, toPath, ftpClient.getReplyString(), ex);
        } catch (IOException e) {
            logger.error("FTP移动文件失败：{} -> {},{}", fromPath, toPath, ftpClient.getReplyString(), e);
        } finally {
            returnInstance(ftpClient);
        }
        return false;
    ...
    public static boolean copyFile(String fromPath, String toPath, boolean override) {
        ...
        try {
            ...
            logger.error("FTP复制文件失败：{} -> {},{}", fromPath, toPath, ftpClient.getReplyString(), ex);
        } catch (IOException e) {
            logger.error("FTP复制文件失败：{} -> {},{}", fromPath, toPath, ftpClient.getReplyString(), e);
        } finally {
            returnInstance(ftpClient);
        }
        return false;
    ...
    private static UploadStatus _breakPointUpload(FTPClient ftpClient, InputStream stream, String remote, int offset) {
        try {
            ...
            if (null == out) {
                logger.debug("断点续传失败：{}", ftpClient.getReplyString());
            ...
            logger.error("FTP上传文件失败：{},{}", remote, ftpClient.getReplyString(), e);
            return UploadStatus.Upload_From_Break_Failed;
        }
    }
    ...
    private static int _gotoRootDir(FTPClient ftpClient) throws IOException {
        ...
        logger.debug("回到根目录：{}", ftpClient.getReplyString());
        return res;
    }
        ...
        try {
            ...
            if (!directory.equalsIgnoreCase("/") && !ftpClient.changeWorkingDirectory(directory)) {
                ...
                while (true) {
                    ...
                    logger.debug("进入目录：{}", subDirectory);
                    if (!ftpClient.changeWorkingDirectory(subDirectory)) {
                        logger.debug("进入目录失败：{}{}", subDirectory, ftpClient.getReplyString());
                        int rs = ftpClient.mkd(subDirectory);
                        if (FTPReply.isPositiveCompletion(rs)) {
                            ftpClient.changeWorkingDirectory(subDirectory);
                        } else {
                            logger.debug("创建目录失败:{},{}", subDirectory, ftpClient.getReplyString());
                            return false;
                        }
                    }
            ...
            logger.error("FTP创建目录失败：{},{}", remote, ftpClient.getReplyString(), ex);
            return false;
        }
    }
    ...
    private static boolean _isFileExist(FTPClient ftpClient, String remote) {
        ...
        try {
            ...
            logger.error("查看文件是否存在失败：[{}-{}]", ftpClient.getReplyString(), remote, e);
        }
