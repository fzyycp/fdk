...
public class FdkCaptchaValidateFilter extends AccessControlFilter {
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue)
		...
		request.setAttribute(FdkCaptchaConst.ATTRIBUTE_KEY_CAPTCHA_ENABLED, fdkCaptchaProperties.isEnable());

		HttpServletRequest httpServletRequest = WebUtils.toHttp(request);
		// 2、判断验证码是否禁用 或不是表单提交（允许访问）
		if (!fdkCaptchaProperties.isEnable() || !"post".equalsIgnoreCase(httpServletRequest.getMethod())) {
			return true;
		}
		// 3、此时是表单提交，验证验证码是否正确
